...
2. **Evaluate** — `GetItemDeleteReason` returns `"quest"`, `"consumable"`, `"equipment"`, `"gray"`, or `nil`. Quest items must additionally pass `IsQuestCompleted`; consumables must be outgrown per `GetConsumableEraseLevel` (see *Eraser Categories*).
...
Note that the palette keys `ON` and `OFF` are *colors*, unrelated to the locale keys `ENABLED` / `DISABLED` that carry the tooltip's state words. `GetColor("ON") .. L["ENABLED"]` is correct and not a typo.
...
elseif consumableDatabase[itemId] then   -- outgrown, per GetConsumableEraseLevel
...
The normal rule is a ten-level gap. The under-5 carve-out exists because the starter food and drink a character is handed in the first few minutes are worthless long before level 11 — by 5 they have already been replaced, so they should not squat in the bags waiting for the general rule. The gap is deliberate rather than smoothed: a `useLevel` 4 item clears at 5, a `useLevel` 5 item not until 15.

The level comes from the data file and never from `GetItemInfo` — see *Item Data Caching* for why that matters.
...
Magic Eraser uses **AceDB-3.0** with one managed table, `MagicEraserDB`, initialized in `ns:OnPlayerLogin`.

**Model: Per-Character.** `AceDB:New` deliberately omits the shared-Default third argument, so each character gets its own `"Name - Realm"` profile. **Reset Profile therefore clears only the active character's ignore list; every account-wide setting survives it.** That is the fact to check before adding a setting: user settings go in `global`, and only state that genuinely differs per character belongs in `profile`.
...
`## SavedVariablesPerCharacter: MagicEraserCharDB` is still declared in the TOC, but only as a legacy migration source — it is read once and cleared. The declaration is scheduled for removal after 2026-08-15 (see the MIGRATION comment in the TOC).
...
All migrations run once in `ns:OnPlayerLogin`, guarded so they self-disable and are safe across every character. Every step is tagged `remove after 2026-08-15`, the factory's single fixed cutoff.
...
2. **Profile → global rehome** — the first AceDB build kept settings on one shared profile; a non-nil raw read for any settings key means the user set it, so it moves up to `global` and is cleared from the profile. Idempotent across characters.
3. **Shared profile → per-character profiles** — moves this character onto its own `"Name - Realm"` profile, carrying its list out of the old shared `Default.ignoreLists` keyed table into a flat `profile.ignoreList`, then drops the obsolete keyed table. A non-empty flat `ignoreList` still sitting on the old `Default` marks a first-AceDB profile whose merged buckets are unreliable; those are discarded and the character re-seeds from its own pre-AceDB list instead.
4. **Auto-Vend message reset** — the Line Item/Summary Only rollout forces Auto-Vend messages on once for everyone via the `autoVendSummaryMigrated` marker, which has no default entry so AceDB never strips it and the reset cannot re-fire.
5. **Stray macro cleanup** — the removed Delete Macro feature left an account-wide `- Eraser` macro; it is deleted once, combat-guarded (`DeleteMacro` is protected), and marked done only after a real pass, so a login-in-combat retries next time.

## Adding a New Trash Item

1. **Consumable** — append `[itemId] = { useLevel }` to `Data/Consumables.lua` with a name comment, where `useLevel` is the level the item becomes usable. That level is carried in the data on purpose and is never read from `GetItemInfo`, so the gate works on a cold item cache and can't shift between client versions. Eligibility is `useLevel + 10`, or level 5 flat when `useLevel` is under 5 (`GetConsumableEraseLevel` in `Eraser.lua`).
...
Keep entries alphabetized by the trailing name comment so diffs stay readable. Per the style guide, these tables are appended to, never reformatted or reproduced wholesale, and each carries either its originating SQL query or a `-- TODO: Add SQL Query` marker until that query is filled in. `Consumables.lua` carries the real query; `Quest-Items.lua` and `Equipment.lua` still carry the marker — leave those markers in place.
...
A new setting needs no migration. One is needed only when an *existing* key changes meaning or home; those go in `ns:OnPlayerLogin`, guarded by a marker with no default entry (so AceDB never strips it) and dated in a `MIGRATION` comment carrying the fixed cutoff.
...
- **Values that equal English** — legitimate only for the add-on title, brand names (CurseForge, GitHub, Discord, Wago), the `/eraser` literal, and true homographs verified per case (`OPTIONS_IGNORE_GLOBAL` is "Global" in esES/esMX/frFR/ptBR because *global* is native vocabulary there, while deDE/itIT/koKR/ruRU/zhCN/zhTW translate it). Anything else left in English is a bug.
...
- **Editing an ignore list from outside the options panel**: an open Ignore List panel will not notice. The minimap mutators fire `NotifyChange` themselves; any new out-of-panel writer must do the same.
...
- **New saved-variable fields** seed defaults through `ns.DATABASE_DEFAULTS` and let AceDB apply them; never hand-merge or overwrite user values.
- **Database edits** keep the column-header comment and the SQL query or `TODO` marker; don't reformat existing rows.
- **Migration discipline** — migrations are one-time, guarded, and carry the fixed `remove after 2026-08-15` cutoff; don't remove one before that date and don't extend it past.
...
- **Commit and PR descriptions require a User Story.** Don't just say "I changed X" or "I fixed Y" — frame the change by who it helps and why.
  ...
  **Example:** *As a player levelling an alt, I wanted the starter bread and water handed out at level 1 to be flagged as trash before level 11 so my opening bag slots weren't hostage to food I replaced hours ago. This change adds `GetConsumableEraseLevel`, which clears any consumable with a use level under 5 at level 5 instead of the usual ten-level gap.*
