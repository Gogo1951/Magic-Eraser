...
├── MagicEraser.toc                 Load order
├── Bindings.xml                    The one key binding, found at the root by the client and never listed in the TOC
├── Data/
│   ├── Data.lua                    Locale init and every shared constant, from the bag range to the class reagents
...
│   ├── Erase-List.lua              Both erase lists, their per-scope writes, the class-reagent seed and Restore Defaults
│   ├── Eraser.lua                  Scan, evaluate, rank, erase, the value cap, the confirmation dialog and the quest-item alerts
...
│   ├── Key-Bindings.lua            The binding's display-name global and its handler, the mini-map Left-Click on a key
...
`.github/`, `.gitattributes`, `.gitignore`, `.luacheckrc`, `.pkgmeta` and `LICENSE` are repo-only; the packager strips them, so an installed copy does not carry them. One TOC serves both flavors, with a single combined `## Interface` line. `Data/`, `Features/` and `Options/` are listed in TOC load order rather than alphabetically, because load order is what makes a fragment's builder exist by the time a panel calls it. There are no deprecated or dead files: everything in those three directories is listed in the TOC and reachable at runtime. `Bindings.xml` has no TOC line on purpose, because the client finds it at the root on its own; see *Key Bindings*. `Includes/Libraries/` is rewritten by the release workflow from `.pkgmeta` on every tag, so a hand edit there is overwritten by the next release.
...
| `DELETE_ITEM_CONFIRM` | `ns:OnDeleteItemConfirm` | Turns the client's typed delete prompt into a plain Yes/No on a hand delete, for items in the chosen scope |
...
Three functions refuse outright rather than deferring:
...
`ns.LAST_BAG_INDEX` in `Data/Data.lua` states the carried-bag range once, as `NUM_BAG_SLOTS` with a numeric fallback so a comparison against a global the client never defined cannot error. Every bag walk and every is-this-a-carried-bag test reads it: the eraser's candidate scan, quest-starter walk and turn-in walk, the vendor scan, the free-slot count, both tooltip hook paths, and the bank's first bank-bag index.
...
The eraser also keeps a short-lived candidate cache (`cachedItem` plus `isCacheValid`) invalidated on bag update, level-up, any list edit, profile switch, deletion, quest turn-in, a value-cap change, the Erase List seed, and a retrieval pass that moved something. `ns:RefreshDisplay` invalidates it too before recomputing, so every repaint is a fresh scan and the cache serves only the reads between two repaints: an opened tooltip, a click on the button, the Clutter Report totals.
...
When both the master toggle and the matching per-reason toggle are on, `RunEraser` shows the `MAGICERASER_CONFIRM_ERASE` static popup instead of erasing directly. The key binding enters through the same `RunEraser`, so a key press asks exactly when a click would. The candidate is passed as the dialog's `data` so each showing acts on the exact item the player saw, and `PerformErase` re-validates the slot (the cursor's item id must match) before deleting, so a slot that shifted while the dialog was open aborts rather than deleting the wrong item. `preferredIndex = 3` avoids tainting the shared dialog stack.

## Manual Delete Assistance

The counterpart to the section above, and the only feature in the add-on that acts on a delete *Magic Eraser* did not initiate. When the player drags an item out of their bags themselves and the client asks them to type a word to confirm, `Manual-Delete.lua` turns that prompt into a plain Yes/No, leaving one click.
...
**The feature ships on, and its scope ships conservative.** `manualDeleteAutoFillEnabled` defaults to true, so a fresh install simplifies the prompt without being asked, and `manualDeleteNoValueOnly` defaults to true as well, which is what keeps that safe: only an item no vendor will buy is ever simplified. Because AceDB strips default-valued keys at `PLAYER_LOGOUT`, flipping either default reaches existing players too, not just new installs, so a change to one belongs in the release notes.

**Scope is one dropdown with two values.** **Applies To** offers **All Items** or **No Sale Value**. The narrow value covers only items no vendor will buy, the set with no disposal route other than deleting it, so anything with a sale price, a rare or epic the player could have vendored included, keeps the full confirmation. The section's description quotes the confirm word through a `%s` filled from the client's own `DELETE_ITEM_CONFIRM_STRING`, so it names exactly the word the player's dialog asks for, in their locale.
...
The eraser's own deletes never reach here. `ns:PerformErase` calls `DeleteCursorItem` only inside the player's own input, a mini-map click, a press of the key binding, or the Yes on the eraser's confirmation, and that input is the hardware event, so the client never asks. That is both why the eraser carries the separate confirmation described above and the clue to the constraint at the top of this section.
...
**Message modes.** All Auto-Vend chat routes through the file-local `PrintVendMessage`, a no-op unless `ns.db.global.autoVendMessagesEnabled` is set. With messages on, the **Line Item / Summary Only** dropdown (`autoVendSummaryEnabled`) chooses whether a per-item `L["SOLD_ITEM"]` line prints as each sale is confirmed; the closing summary prints in both modes on the deferred `MERCHANT_CLOSED` flush, as `L["SOLD_SUMMARY_ONE_ITEM"]` when a single item sold, `L["SOLD_SUMMARY_ONE_SLOT"]` when a single stack did, and `L["SOLD_SUMMARY"]` otherwise (see *Localization*). Per-visit totals accrue for every newly confirmed sale regardless of the current mode, so flipping the dropdown mid-visit still produces a correct closing line.
...
`FinishPass` is the single exit, however the pass ended. It prints the summary, `L["BANK_RETRIEVED"]` or its `_ONE_ITEM` / `_ONE_SLOT` variant, only when something was confirmed moved, then resets the totals and drops any unconfirmed move, so the second call (the pass finishing, then `BANKFRAME_CLOSED` arriving) stays silent and nothing leaks into the next pass.
...
**No mini-map click.** All five click combinations are taken, so the Erase List is edited from its panel only. There is no `ToggleErase` or `ClearEraseList`, and consequently no out-of-panel writer that has to fire its own `NotifyChange`: the shared item-list builder covers every edit path this list has.
...
One binding ships, defined in `Bindings.xml` at the add-on root: `MAGICERASER_ERASE`, listed as **Erase Lowest-Value Item** in the **Magic Eraser** section of the game's Key Bindings list. It is the mini-map button's Left-Click on a key and nothing more. Its handler, `MagicEraser_Erase` in `Features/Key-Bindings.lua`, calls `ns:RunEraser`, so the combat refusal, the Mini-map Eraser Confirmation, Maximum Value to Erase and both Ignore Lists hold for a key press exactly as they do for a click. Keep it on that one entry point: a handler that picked and erased an item itself would drift from the button the first time either changed. Keep it synchronous as well, since a key press carries the hardware event `DeleteCursorItem` needs (see *Manual Delete Assistance*), and a timer between the press and the delete would lose it.

A key press is riskier than the click it copies, because it comes with no preview. The button's icon and tooltip show the next item before the player commits, while a key shows nothing, so a stray press goes after whatever is next in line. That is why the General panel's Key Bindings section tells the player it is use at your own risk. The section is a pointer rather than a control, since a binding cannot be set from an AceConfig panel, and it names the binding with the same `L["BINDING_ERASE"]` the Key Bindings list shows, so the two never disagree.

The binding needs two globals, both demanded by the binding system and both sanctioned by Style Guide → FILE STRUCTURE: `MagicEraser_Erase`, because `Bindings.xml` can only call a global function, and `BINDING_NAME_MAGICERASER_ERASE`, the display name the Key Bindings list looks up from the element's `name` attribute. Both are in `.luacheckrc`'s `globals`.

`Bindings.xml` is **found at the add-on root by the client itself and must never be listed in the TOC**. Listing it routes the file through the generic UI XML parser, which does not know the `<Bindings>` node and rejects the whole file with `Unrecognized XML: Bindings` warnings, so the binding never appears. Its root element is `<Bindings>`, not a `<Ui>` wrapper, which fails the same way. The `category` attribute is what draws the **Magic Eraser** section in the Key Bindings list; without it the binding loads but has nowhere to appear.
...
- **API Endpoints**, `ns.DIAGNOSTIC_API_CHECKS`, kept one-to-one with the APIs the add-on calls or guards; existence and shape checks only. Every modern/legacy pair the add-on guards on is listed as both halves. **A `[FAIL]` on one half of a pair is the report working, not a defect**: the pair is what tells a bug report which branch that client took, so the failing half is the one carrying the answer and is never dropped. The tooltip pair is the worked example, `[FAIL]` on Era and `[PASS]` on TBC Anniversary, which is the measurement behind the two hook paths in *Item Tooltip Warnings*. The six Manual Delete rows cover the two client strings and four dialog methods that feature relies on, so a report of a prompt that was not simplified names whichever one this client lacks.
...
Sub-options are built by `ns.OptionsSubRow` and `ns.OptionsSubLabel` in `Options-Utilities.lua`: an unnamed inline group whose first arg is a blank indent cell, then the controls, with `hidden` on the group and never on the members. They live in Utilities rather than one panel file because four fragments across both settings panels build sub-rows: Auto-Vend, Manual Delete Assistance, the Eraser pair and Bag-Space Warnings. Per-control widths inside a sub-row are sized to their captions with slack, never to the full row budget, because a row sitting on the wrap boundary tips its control onto its own line and strands the indent above it.
...
`/eraser` opens the Options Interface. Registration is in `Options.lua` (`SLASH_MAGICERASER1` plus `SlashCmdList.MAGICERASER`), and the handler does nothing but call the opener. Past the combat gate described in *Combat Lockdown*, `AddToBlizOptions` returns `(frame, categoryID)` and **both are captured** at the root panel's registration, as `ns.GeneralPanel` and `ns.GeneralCategoryID`; `ns:OpenOptionsPanel` routes `Settings.OpenToCategory(<captured categoryID>)` and reaches `AceConfigDialog:Open` only as a last resort a correctly-routed add-on never sees. There is no legacy `InterfaceOptionsFrame_OpenToCategory` route and none is wanted: the vendored AceConfigDialog registers every panel through the Settings API (`Settings.RegisterCanvasLayoutCategory` for the root, its subcategory counterpart for the children), so a client old enough to need that fallback would already have failed at `ns:RegisterOptionsPanels`. Never look the category up by display name: AceConfigDialog only aliases the category ID to the panel's name on clients lacking `C_SettingsUtil.OpenSettingsPanel`, so a name lookup returns `nil` on TBC Anniversary and the panel opens as a floating standalone window instead of docking into Blizzard's settings.
...
There is no refill-on-empty logic. The curated item databases (`AllowedDeleteQuestItems`, `AllowedDeleteQuestStartingItems`, `AllowedDeleteConsumables`, `AllowedDeleteEquipment`) are static Lua tables shipped in `Data/`, not saved variables, so they can never be emptied at runtime. The saved lists that *can* be emptied are the ignore and erase lists. The ignore lists ship no defaults at all, and the per-character Erase List's defaults, the class-reagent seed, go in once per character and again only on Restore Defaults, never because the list is empty: an empty list is what a player who cleared it asked for, and `profile.eraseListSeeded` is what tells that apart from a list never seeded. See *The Class-Reagent Seed*.

There is no migration chain; the add-on carries no migration code. What a future change to saved data owes is under *Contributing*.

## Adding a New Trash Item

1. **Consumable.** Append `[itemId] = { useLevel }` to `Data/Consumables.lua` with a name comment, in the right kind block (Food, Water, Both, Alcohol), where `useLevel` is the level the item becomes usable, which the file's query derives as item level minus 10, never below 1. That level is carried in the data on purpose and is never read from `GetItemInfo`, so the gate works on a cold item cache and cannot shift between client versions. Eligibility is `useLevel + 10`, or level 5 flat when `useLevel` is under 5 (`GetConsumableEraseLevel` in `Eraser.lua`).
2. **Equipment.** Append `[itemId] = true` to `Data/Equipment.lua` with a name comment, in the **WEAPONS** or **ARMOR** block and then the right expansion block inside it. White vendor trash, no level gate, soulbound included. A weapon qualifies on `class = 2` with `dmg_max1 > 0` and a subclass that is not a profession tool (14 and 20); armor on `class = 4`, a worn subclass (1 to 6), and `armor > 0`. The file carries both queries. Remember that the runtime branch also requires the live client to report the item as white, so a row that turned green in a later expansion simply does nothing on the client where it is green. The table also carries 28 items that are white in Era but green in the WotLK database the queries read, so no query returns them; the note above the table accounts for them, and a regeneration has to carry them over.
3. **Quest item.** Append `[itemId] = { questId, ... }` to `Data/Quest-Items.lua`. Key the row to the quest whose completion makes the item safe, which is the *latest* point the item could still be needed: in the linear case the quest that takes it at turn-in (`ReqItemId` in `quest_template`), never the one that hands it out; where mutually exclusive sibling quests converge on a final quest, that final quest alone, with no sibling listed; where the granting quest is the last step, that quest and not the one before it. Treat one quest title repeated across several ids as a signal to check for siblings, and confirm chain order from the quest pages rather than Wowhead's previous/next widget, which misreports siblings as sequential. The header's two queries find candidates and prove which items can still be in a bag after their quest is done; they never produce rows to paste. A wholesale regeneration from them has already keyed rows early enough to erase items a later quest still needed, so audit a proposed change by comparing the parsed item-to-quest pairs before and after rather than reading a raw diff.
...
6. **Neither of the above.** Anything a player wants gone that no query can express belongs on their own Erase List, not in a `Data/` table. A row no query can express has no durable home in a regenerated file: the two Shaman reagents were hand-added to `Equipment.lua` and the next regeneration dropped them, which is the failure the Erase List exists to prevent.

Nearly every row in `Quest-Starting-Items.lua` also appears in `Quest-Items.lua`. That overlap is deliberate; see *Eraser Categories* for why the two tables share one branch. The second Quest-Items query refuses to propose a starter for the same reason: its row would be the starter's own quest id with the masks stripped off, which could never fire first.
...
- **Placeholders.** `%s` and `%d` count, type and order must match `enUS` per key in every locale, or the string crashes at runtime. Lua's `string.format` has no positional placeholders, so a translation may move the words around its placeholders but never the placeholders themselves: swapping two of them prints each value in the other's place. Three shapes need care.
  - The `%s%s` link-plus-stack pair in `CONFIRM_ERASE`, `ERASED_ITEM`, `ERASED_ITEM_WITH_VALUE`, `ERASED_ITEM_FROM_QUEST`, `ERASED_ITEM_QUEST_UNAVAILABLE` and `SOLD_ITEM` is a single unit rather than two fields. The second placeholder is `" x5"` or the empty string, so a translation that separates the two lands the stack count away from its item.
  - Counts and money arrive as formatted strings, counts through `ns:FormatCommaNumber` (`"1,234"`) and money through `ns:FormatCurrency`, so every one of them is a `%s`. Turning a count into `%d` works until the first count past 999, then errors on the comma.
  - `OPTIONS_MANUAL_DELETE_DESCRIPTION` carries one `%s`, filled with the client's own `DELETE_ITEM_CONFIRM_STRING`, so the description quotes exactly the word the player's delete dialog asks for. A translation keeps the `%s` rather than typing the word in.
- **A count of one gets its own complete string.** Every counted message branches on exactly one rather than printing "1 bag slots": `SOLD_SUMMARY_ONE_ITEM` and `SOLD_SUMMARY_ONE_SLOT` beside `SOLD_SUMMARY`, the same pair beside `BANK_RETRIEVED`, `CLUTTER_ITEMS_ONE` and `CLUTTER_SLOTS_ONE` in the mini-map tooltip, and `BAGS_FULL_NUDGE_ONE` between `BAGS_FULL` at zero and `BAGS_FULL_NUDGE`. The one-item case is tested first, because one item is always one slot. Nothing branches past one, so the general string has to read correctly for every count from two up; a language with more than two plural forms phrases it as a label and a number (ruRU's `CLUTTER_ITEMS` is `"(предметов: %s)"`, "items: %s") rather than declining the noun to agree with a number it cannot see.
- **Not localized:** `ns.DiagnosticsStrings` (developer-facing), the AceConfig registry names in `ns.OPTIONS_REGISTRY`, `ns.LIST_SCOPE_GLOBAL`, `ns.ITEM_LINK_WIDGET_TYPE`, AceDB profile names, which are `"Name - Realm"` character keys, and the `category` attribute in `Bindings.xml`, which is the add-on's own name.
...
- **Erasing a slot that is not what the scan saw**: a second click can land before the client has finished with the first, and a confirmation dialog can outlive the slot it named. `PerformErase` checks that `GetCursorInfo` holds the expected `itemId` before `DeleteCursorItem`, and on a mismatch prints `L["CURSOR_TOO_FAST"]` and clears the cursor rather than deleting whatever it picked up.
- **Answering the client's delete dialog from code**: calling `StaticPopup_OnClick`, the accept button or `OnAccept` without the player's click drops the delete while the dialog hides itself anyway, leaving the item alive and its prompt gone. Manual Delete Assistance only ever clears the typing; the player's own click on Yes is what deletes.
...
- **Queuing an item a vendor will not buy**: `ScanAndVend` filters on `sellPrice > 0` at scan time. Most quest items have no sell price, so they stay in the bags for the eraser instead of being handed to a merchant that cannot take them.
...
- **A count of one read through a plural string**: "Sold 1 items (1 bag slots)". Branch on the count and give the singular its own complete key, the way the vend and bank summaries, the Clutter Report and the bag-space countdown do; see *Localization*.
- **Listing `Bindings.xml` in the TOC**: the UI parser rejects the file and the binding never appears. The client loads it from the add-on root on its own; see *Key Bindings*.
...
- **Saved-data changes carry a migration.** The add-on ships no migration code today. A change to the shape, name or scope of saved data ships with its own migration for the data players already have, tagged `-- MIGRATION (remove after YYYY-MM-DD)` 30 days past its release and deleted once that date passes, per Style Guide → SAVED VARIABLES → Migration Windows. A key retired with nothing to carry over is nil'd explicitly (`ns.db.global.oldField = nil`).
