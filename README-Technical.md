...
├── MagicEraser.toc               Load order; one TOC covers Era and TBC Anniversary
├── Data/
│   ├── Data.lua                  Locale init, identity, links, options grid, palette, race and class bits
│   ├── Default-Settings.lua      The AceDB defaults table, global and profile scopes
│   ├── Quest-Items.lua           Items a turn-in consumes, keyed to the quests that take them
...
│   ├── Item-Tooltips.lua         Adds the will-erase or protected line to bag-item tooltips
│   ├── Diagnostics.lua           Report builders, event log, API and event probes, taint log
│   └── Minimap-Button.lua        LDB object, tooltip composition, click handlers, ns:RefreshDisplay
├── Includes/
│   ├── Images/
│   │   └── Magic-Eraser.tga      The TOC's IconTexture
│   └── Libraries/                Vendored Ace3 stack plus LibDataBroker and LibDBIcon
├── Locales/
...
`.github/`, `.gitattributes`, `.gitignore`, `.luacheckrc`, `.pkgmeta` and `LICENSE` are repo-only; the packager strips them, so an installed copy does not carry them. There are no deprecated or dead files: everything under `Data/`, `Features/` and `Options/` is listed in the TOC and reachable at runtime.
...
|-------|---------|---------|
| `PLAYER_LOGIN` | `ns:OnPlayerLogin` | AceDB init, options registration, profile callbacks, LibDBIcon registration, welcome print, the two alert baselines, tooltip-hook install |
...
The eraser also keeps a short-lived candidate cache (`cachedItem` plus `isCacheValid`) invalidated on bag update, level-up, ignore-list change, profile switch, deletion, quest turn-in, a value-cap change, and a completed retrieval pass.
...
Magic Eraser writes no macros and sends no chat, so neither the 255-character macro limit nor the 255-byte chat limit constrains any of this: every player-visible string is a local `print` or an options-panel label. The practical ceiling is visual, since a translated label has to fit its widget.
...
- **Keeping locales in sync.** Every non-English file carries a translation of the same key set, and AceLocale falls back to English via `__index` for anything missing at runtime. Translating each `enUS.lua` key into every locale is the job of the Localization pass, so do not hand-edit the other locales during ordinary work. A renamed key leaves harmless orphans in the translated files until that pass runs, and a retired key name is never reused.
...
- **Database edits** keep the column-header comment and the SQL query, and do not reformat existing rows.
...
- **Output length**: Magic Eraser writes no macros and sends no chat, so the 255-character macro limit and 255-byte chat limit do not apply. If a change ever adds a sent message, measure the decorated line in bytes and check the widest-encoding locale (ruRU, not automatically deDE).
